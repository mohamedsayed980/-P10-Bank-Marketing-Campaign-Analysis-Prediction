"""
Repo_10_Bank_Marketing — ML_Models.py  (5 Tabs)
Author : Mohamed · M3
...
st.set_page_config(page_title="ML Models · Bank Marketing · M3",
...
with st.sidebar:
    ...
    st.markdown("Bank Marketing · 5 Tabs")
    st.divider()
    st.markdown("### 📂 Dataset")
    ...
    if _uploaded is not None:
        ...
        st.info("Using default: bank_clean.csv")
    st.divider()
...
if df.empty:
    st.warning("⚠️ No data. Run P10_clean_data.py then upload bank_clean.csv.")
    st.stop()

if "y_binary" not in df.columns:
    st.error("❌ Wrong file — upload bank_clean.csv (output of P10_clean_data.py), not the raw file.")
    st.stop()
...
NUM_FEATS = [c for c in ["age","campaign","pdays","previous","day","month_num",
                          "has_debt","prev_success","was_contacted_before",
                          "high_campaign_effort"]
             if c in df.columns]
...
with tabs[0]:
    ...
    with col1:
        ...
        st.markdown(f"**`{REG_TARGET}`** — predict account balance (€)")
        st.markdown(f"Mean=€{y_reg.mean():,.0f} · Range=€{y_reg.min():,}–€{y_reg.max():,}")
    with col2:
        sec("🎯 Classification Target")
        st.markdown(f"**`{CLF_TARGET}`** — predict subscription (0/1)")
        st.markdown(f"Subscribe rate={y_clf.mean()*100:.1f}% → class_weight='balanced'")
    ...
    warn("Subscribe rate 11.7% → class_weight='balanced' on all · Evaluate F1 + ROC-AUC")
    cc = st.columns(3)
...
with tabs[1]:
    ...
    info("Predicting: **Account Balance (€)**")
    ...
    if not S.get("reg_results"):
        ...
        with col3:
            ...
            ax.scatter(yr_test, pred, alpha=0.2, s=5, color=CLR["primary"])
        ...
        info("Account balance is skewed — tree-based models (RF, GB) handle this better than linear.")
...
with tabs[2]:
    ...
    warn("11.7% subscribe rate — evaluate with F1, Recall, ROC-AUC. NOT accuracy.")
    ...
    if not S.get("clf_results"):
        ...
        with col3:
            ...
            sns.heatmap(cm, annot=True, fmt="d", cmap="Blues",
                        ...
                        yticklabels=["Not Sub","Subscribed"], ax=ax)
            ax.set_xlabel("Predicted"); ax.set_ylabel("Actual")
            ax.set_title(f"Confusion Matrix — {best_clf}")
            plt.tight_layout(); st.pyplot(fig3); plt.close()
            tn,fp,fn,tp = cm.ravel()
            st.markdown(f"**TP={tp}** (correctly identified subscribers) · **FN={fn}** (missed subscribers)")
        ...
        with col4:
            ...
            if hasattr(bm,"predict_proba"):
                ...
                ax2.plot(fpr, tpr, color=CLR["success"], lw=2.5, label=f"AUC={auc_val:.4f}")
                ax2.plot([0,1],[0,1], color=CLR["grey"], ls="--")
                ax2.fill_between(fpr, tpr, alpha=0.1, color=CLR["success"])
                ax2.set_xlabel("FPR"); ax2.set_ylabel("TPR")
                ax2.set_title(f"ROC Curve — {best_clf}"); ax2.legend()
                plt.tight_layout(); st.pyplot(fig4); plt.close()

        insight(f"Best classifier: {best_clf} — AUC > 0.80 expected with RF/GB.")
        warn(f"FN={fn} missed subscribers — in business context, missing a subscriber = lost revenue.")
    ...
    if not S.get("clf_models"):
        ...
        with col1:
            ...
            if hasattr(bm,"feature_importances_"):
                ...
                colors_i = [CLR["success"] if i>=len(imp)-3 else CLR["primary"]
        ...
        insight("prev_success and poutcome_enc typically dominate — previous campaign outcome is king.")
        insight("balance and age rank highly — demographic and financial capacity signals.")
        warn("KNN and SVM (Linear) don't expose importances — use RF/GB for interpretability.")

# ══════════════════════════════════════════════════════════════
# TAB 5 — PREDICT
# ══════════════════════════════════════════════════════════════
with tabs[4]:
    sec("🔮 Tab 5 — Interactive Subscription Probability Scorer")

    if not S.get("clf_models"):
        warn("Train at least one model in Tab 1.")
    else:
        info("Enter client profile to predict subscription probability.")
        ...
        with col1:
            ...
            balance  = st.number_input("Account Balance (€)", -5000, 50000, 1500, 100)
        ...
        with col3:
            sec("📊 Campaign Info")
            campaign  = st.slider("Calls This Campaign", 1, 30, 2)
            ...
            was_contacted    = 1 if pdays != -1 else 0
            high_effort      = 1 if campaign > 3 else 0
            prev_success_val = 1 if poutcome == "success" else 0
            has_debt_val     = 1 if (housing=="yes" or loan=="yes") else 0

            st.metric("Was Contacted Before", "✅ Yes" if was_contacted else "❌ No")
            st.metric("High Campaign Effort", "⚠️ Yes" if high_effort else "✅ No")
            st.metric("Previous Success",     "🌟 Yes" if prev_success_val else "❌ No")
            st.metric("Has Debt",             "⚠️ Yes" if has_debt_val else "✅ No")
        ...
        if st.button("🔮 Predict Subscription Probability", type="primary", use_container_width=True):
            ...
            def enc(col, val):
                vals = df[col].unique().tolist() if col in df.columns else [val]
                le.fit([str(v) for v in vals] + [str(val)])
            ...
            row_dict = {
                ...
                "day":                  day,
                "month_num":            month_num,
                "has_debt":             has_debt_val,
                "prev_success":         prev_success_val,
                "was_contacted_before": was_contacted,
                "high_campaign_effort": high_effort,
                "job_enc":              enc("job",       job),
                "marital_enc":          enc("marital",   marital),
                "education_enc":        enc("education", education),
                "default_enc":          enc("default",   default),
                "housing_enc":          enc("housing",   housing),
                "loan_enc":             enc("loan",      loan),
                "contact_enc":          enc("contact",   contact),
                "month_enc":            enc("month",     month),
                "poutcome_enc":         enc("poutcome",  poutcome),
            }
            input_row = pd.DataFrame([{k: row_dict.get(k,0) for k in S["X_cols"]}])
            input_sc  = S["scaler"].transform(input_row)

            sec("🎯 Subscription Prediction — All Classifiers")
            pred_rows = []
            for name, model in S["clf_models"].items():
                ...
                pred_rows.append({
                    "Model":       name,
                    "Prediction":  "🌟 WILL SUBSCRIBE" if pred==1 else "❌ WON'T SUBSCRIBE",
                    "Probability": f"{prob*100:.1f}%" if prob is not None else "N/A",
                })
            pred_df = pd.DataFrame(pred_rows)
            st.dataframe(pred_df, use_container_width=True)

            sub_votes = sum(1 for r in pred_rows if "WILL" in r["Prediction"])
            verdict   = "🌟 LIKELY TO SUBSCRIBE" if sub_votes > len(pred_rows)/2 else "❌ UNLIKELY TO SUBSCRIBE"
            if "LIKELY" in verdict:
                st.success(f"{verdict} — {sub_votes}/{len(pred_rows)} models predict subscription.")
            else:
                st.error(f"{verdict} — only {sub_votes}/{len(pred_rows)} models predict subscription.")

            st.markdown("---")
            sec("📋 Key Risk / Opportunity Factors")
            factors = []
            if prev_success_val: factors.append(("✅ Opportunity","Previous campaign success — highest conversion signal"))
            if was_contacted:    factors.append(("✅ Opportunity","Previously contacted — warm lead"))
            if balance > 2000:   factors.append(("✅ Opportunity",f"High balance (€{balance:,}) — financial capacity confirmed"))
            if job in ["retired","student"]: factors.append(("✅ Opportunity",f"Job: {job} — high subscription segment"))
            if contact == "cellular": factors.append(("✅ Opportunity","Cellular contact — best performing method"))
            if high_effort:      factors.append(("⚠️ Risk",f"High campaign effort ({campaign} calls) — resistance signal"))
            if has_debt_val:     factors.append(("⚠️ Risk","Has debt burden — reduces investment capacity"))
            if balance < 0:      factors.append(("⚠️ Risk",f"Negative balance (€{balance:,}) — no investment capacity"))
            if poutcome == "failure": factors.append(("⚠️ Risk","Previous campaign failure — lower conversion probability"))

            for tag, text in factors:
                color = "insight" if "Opportunity" in tag else "warn"
                if color == "insight":
                    st.markdown(f'<div class="insight-box"><p>✅ {text}</p></div>', unsafe_allow_html=True)
                else:
                    st.markdown(f'<div class="warn-box"><p>⚠️ {text}</p></div>', unsafe_allow_html=True)
